...
// This is accessible here: https://github.com/SaronMariathasan/saron-portfolio/blob/main/quizzing-server/src/auth.ts  
import { tokenValidator, generateUserSessionId, generateUserId, createToken, invalidateToken } from './helperFunctions.js';
...
function adminAuthRegister(email, password, nameFirst, nameLast) {
  ...
  // allow any characters, as long as there's ≥1 letter, ≥1 digit, and length ≥8
...
function adminUserDetails(token) {
...
function updateAdminUserDetails(token, email, nameFirst, nameLast) {
  // Validate the token
  const userToken = tokenValidator(token);
...
function adminPasswordUpdate(token, oldPassword, newPassword) {
  const userToken = tokenValidator(token);
