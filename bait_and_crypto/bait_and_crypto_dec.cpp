...
int doDecryptDir(char dirname[], struct dirent *dp, const unsigned char key[crypto_secretstream_xchacha20poly1305_KEYBYTES], char *homePath);
...
// function to convert txt file back to pdf
// int textToPdf(char *text_filename, char *pdf_filename) {
//     FILE *fd_txt = fopen(text_filename, "rb");
//     PoDoFo::PdfStreamedDocument pdf(pdf_filename);
//     pdf.Load(pdf_filename);
//     int eof = feof(fd_txt);
//     do {
//         unsigned char *buffer_in[CHUNK_SIZE];
//         int rlen;
//         rlen = fread(buffer_in, 1, sizeof buffer_in, fd_txt);

//     } while (!eof)
//     // initialise buffer_in with CHUNK_SIZE
//     // fread from txt file until eof
//     // with read text, store in pdf doc
//     for (int pn = 0; pn < pdf.GetPageCount(); ++pn) {
//         PoDoFo::PdfPage* curr_page = pdf.GetPage(pn);
//         PoDoFo::PdfContentsTokenizer tok(curr_page);
...
int main()
{
    ...
    if (doDecryptDir(documentsDirname, root, KEY, homePath) != 0)
    {
        return 1;
    }
    return 0;
...
int doDecryptDir(char dirname[], struct dirent *dp, const unsigned char key[crypto_secretstream_xchacha20poly1305_KEYBYTES], char *homePath)
{
    ...
    while (dir)
    {
        ...
        else
        {
            ...
            snprintf(dec_file, MAX_FILE_LEN, "%s/%s.dec", dirname, filename);
            ...
            doDecryptDir(new_dirname, subdp, key, homePath);
        }
    }
    closedir(dir);
}
